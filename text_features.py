...
class PsyText():
    ...
    def __init__(self, language):
        ...
        with open("{}/dictionaries/word_categories_{}.json".format(pwd(), language), "r") as h:
            self.param = json.load(h)
            if "map_pos" not in self.param:
                self.param["map_pos"] = {}
            if "ratios" not in self.param:
                self.param["ratios"] = {}
        with open("{}/dictionaries/sentiment_{}.pkl".format(pwd(), language), "rb") as h:
            self.senti = pickle.load(h)
        with open("{}/dictionaries/mappings_{}.json".format(pwd(), language), "r") as h:
...
def text_features(s, lng="eng", nrm=True, param=None, senti=None, mapping=None, nlp=None):
    ...
    if param is None:
        with open("{}/dictionaries/word_categories_{}.json".format(pwd(), lng), "r") as h:
            param = json.load(h)
    if senti is None:
        with open("{}/dictionaries/sentiment_{}.pkl".format(pwd(), lng), "rb") as h:
            senti = pickle.load(h)
    if mapping is None:
        with open("{}/dictionaries/mappings_{}.json".format(pwd(), lng), "r") as h:
            mapping = json.load(h)
    if nlp is None:
        if lng == "deu":
